...
class musicSearch():
    ...
    def read(self):
        ...
        if self.text in self.Json[str(self.user.id)]:
            ...
            try:
                del self.Json[20][self.Json[str(self.user.id)].index(self.text)]
            except:
                pass
            ...
            try:
                del self.Json[str(self.user.id)][20]
            except:
                pass
            self.Json[str(self.user.id)].insert(0, self.text)
            self.save()
    ...
    def load(self):
        with open("json/musicSearch.json", "r", encoding="UTF-8") as f:
            try:
                ...
                self.Json: list = []
        try:
            return self.Json[str(self.user.id)]
        except:
            return []

    def save(self):
        with open("json/musicSearch.json", "w", encoding="UTF-8") as f:
